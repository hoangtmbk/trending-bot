...
class HuggingFacePapersScout(BaseAgent):
    name = "hf_papers_scout"
    schedule = "0 */6 * * *"
    ...
    def execute(self, ctx: AgentContext) -> AgentResult:
        ...
        sources_cfg = ctx.config.get("sources", {}).get("hf_papers", {})
        if not sources_cfg.get("enabled", False):
            return AgentResult(success=True, message="HF Papers disabled in config")
        ...
        ctx.emit("items_updated", {
            "source": "hf_papers", "count": len(raw_items), "item_ids": item_ids,
        })
        return AgentResult(
            success=True,
            message=f"HF Papers: {len(raw_items)} items collected",
